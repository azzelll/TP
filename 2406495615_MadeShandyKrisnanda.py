...
def olympic_wave(): #Create an Olympic Wave logo
    for i in range(5): 
        if i % 2 == 0 :
            make_circle(start_x+i*move_x,y_top_wave,full_circle,pen_color[i])
        elif i % 2 == 1 :
            make_circle(start_x+i*move_x,y_bottom_wave,full_circle,pen_color[i])
    for i in range(5):
        if i % 2 == 0 :
            make_circle(start_x+i*move_x,y_top_wave,-oneeighth_part,pen_color[i])
            make_circle(start_x+i*move_x,y_top_wave,half_circle,pen_color[i])
        elif i % 2 == 1 :
            make_circle(start_x+i*move_x,y_bottom_wave,-oneeighth_part*3,pen_color[i])
            make_circle(start_x+i*move_x,y_bottom_wave, half_circle,pen_color[i])

def olympic_line(): #Make a straight Olympic logo
    for i in range(5):
        make_circle(start_x+i*move_x,y_line, full_circle,pen_color[i])
    for i in range(5):
        make_circle(start_x+i*move_x,y_line,-quarter_circle,pen_color[i])
        make_circle(start_x+i*move_x,y_line, half_circle,pen_color[i])
